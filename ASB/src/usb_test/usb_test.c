...
#define USB_LOOPBACK_PIN1		PAD_GPIO_A + 17
#define USB_LOOPBACK_PIN2		PAD_GPIO_A + 18
#define USB_LOOP_PWR_EN			PAD_GPIO_A + 19
#define USB_OTG_HOST_TEST		PAD_GPIO_A + 20
#define USB_OTG_EXTHUB_RESET	PAD_GPIO_E + 30
...
int main(int argc, char **argv)
{
	...
	for (index = 0; test_count > index; index++) {
		...
		if(gpio_export(USB_OTG_EXTHUB_RESET)!=0)
		{   
		    printf("gpio open fail\n");
		  	ret = -ENODEV;
			goto err;
		...
		gpio_dir_out(USB_OTG_EXTHUB_RESET);
		...
		switch(test_mode)
		{
			...
			case 2:
				...
				break;
			case 3:
				// otg host ext hub reset
				gpio_set_value(USB_OTG_EXTHUB_RESET, 1);
				usleep(100000);
				gpio_set_value(USB_OTG_EXTHUB_RESET, 0);
				usleep(100000);
				gpio_set_value(USB_OTG_EXTHUB_RESET, 1);

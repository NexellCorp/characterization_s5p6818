...
int simple_test = 0;				/* simple test */

int g_speed  = DEF_SPEED;
char *eth_argp[] = {
	"./ethtool", "-s", INTERFACE_NAME, "speed", "100", "duplex", "full", "autoneg", "on"
};
char *eth_def[] = {
...
void get_src_mac_addr(char *interface, unsigned char *src_mac)
{
	int sd;
	struct ifreq ifr;
...
void print_usage(void)
{
	...
	fprintf(stderr, "                [-s speed]  100/1000 (default 100)\n");
	fprintf(stderr, "                [-k skip packet read/write]\n");
...
int parse_opt(int argc, char *argv[])
{
	...
	while ((opt = getopt(argc, argv, "s:C:kTdh")) != -1) {
		switch (opt) {
			case 's':
			   g_speed			= strtoul(optarg, NULL, 10);
			   opt_s			= 1;
			   break;
			case 'C':
				_repeat			= atoi(optarg);
	...
	if (g_speed != 10 && g_speed != 100 && g_speed != 1000)
		goto _exit;
...
int driver_loaded_test(void)
{
	struct stat sb;

	if (stat(GMAC_SYS_DEV, &sb) == -1) {
		perror(" GMAC device not created.\n");
		return -1;
	}

	if (stat(GMAC_SYS_DRV, &sb) == -1) {
		perror(" GMAC driver not loaded.\n");
		return -1;
	}

	#if 0
	if (stat(GMAC_SYS_PHY, &sb) == -1) {
		perror(" PHY driver not loaded.\n");
		return -1;
	}
	#endif
...
int phy_reg(int write, unsigned reg, unsigned val)
{
	char sysCmd[128];

	if (write)
		sprintf(sysCmd, "echo %s %d %x > " PHY_SYS_REG "\n",
				"write", reg, val);
	else
		sprintf(sysCmd, "echo %s %d    > " PHY_SYS_REG "\n",
				"read",  reg     );
...
int loop_mode(int speed)
{
	...
	phy_reg(PHY_W, 31, 0x0);
	phy_reg(PHY_W, 0,  0x1140);

	usleep(500000);


	/* PHY - loopback setting */
	phy_reg(PHY_W, 31, 0x0);
	phy_reg(PHY_W, 0,  0x8000);
	sleep(1);
	phy_reg(PHY_W, 0,  BMCR);
...
int main(int argc, char **argv)
{
	...
	/*  simple test for svt */
	if (simple_test) {
		if (driver_loaded_test() < 0)
			exit(EBADMSG);

		printf("    gmac result =>        success\n");
		exit(EXIT_SUCCESS);
	}
	...
	/* PHY - test phyreg sys */
	if (path_check(PHY_SYS_REG) < 0)
		exit(-1);


	/* set loopback mode and send/receive packet */
	for (i = 1; i <= g_repeat; i++)
		if (!loop_test())
			break;

...
static int  fb_mmap(int fd, unsigned long *base, unsigned int *len);
static void fb_munmap(unsigned long base, unsigned int len);
static int  fb_get_resol(int fd, int *w, int *h, int *pixbit, int *bufnum);
static void fill_color(void *base, int xres, int yres, unsigned int pixbyte, unsigned int color);
...
int main(int argc, char *argv[])
{
	...
	printf("%s: virt = 0x%08lx, %8d (%4d * %4d, %d bpp, %d buffers) ....\n",
	...
	for (i = 0; fb_bufs > i; i++) {
		unsigned long base = PAGE_SIZE_ALIGN(fb_base + i*(fb_xres * fb_yres * fb_pixbit/8), PAGE_SIZE);
		fill_color((void *)base, fb_xres, fb_yres, fb_pixbit/8, o_color);
	}
...
static void pixel_565to8888(void *base, int xpos, int ypos,
			int width, int height, unsigned int color)
{
...
static void pixel_888to565(void *base, int xpos, int ypos,
			int width, int height, unsigned int color)
{
	*(unsigned short*)(base + (ypos * width + xpos) * 2) = (unsigned short)RGB888TO565(color);
}

static void pixel_888to888(void *base, int xpos, int ypos,
			int width, int height, unsigned int color)
{
...
static void pixel_888to8888(void * base, int xpos, int ypos,
			int width, int height, unsigned int color)
{
	*(unsigned int*)(base + (ypos * width + xpos) * 4) = (0xFF000000) | (color & 0xFFFFFF);
}

static void (*PUTPIXELTABLE[])(void *, int, int, int, int, unsigned int) = {
	pixel_555to565,
	pixel_565to888,
	pixel_565to8888,
	pixel_888to565,
...
static void fill_color(void *base, int xres, int yres, unsigned int pixbyte, unsigned int color)
{
	void (*pixel_)(void *, int, int, int, int, unsigned int) = NULL;
	int x = 0, y = 0;

	// 888to565 or 888to888
	pixel_ = PUTPIXELTABLE[3 + pixbyte - 2];

	printf("Fill FB=0x%p, x=%4d, y=%4d, bpp=%d, col=0x%08x\n",

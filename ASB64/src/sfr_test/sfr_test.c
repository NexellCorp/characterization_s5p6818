...
static unsigned int iomem_map (unsigned long phys, unsigned int len);
static void 		iomem_free(unsigned long virt, unsigned int len);
...
int main(int argc, char **argv)
{
	...
	unsigned long virt = 0, phys = 0, size = 0;
	...
	for (i = 0; TABLE_SIZE > i; i++) {
		...
		if (! IO_VIRT(io,i))
			printf("Fail: mmap phys 0x%08lx, length %ld\n", IO_PHYS(io,i), IO_SIZE(io,i));
	}
	...
	if (test_on) {
		for (index = 0; test_count > index; index++) {				
			if (opt_w) {
				printf("\n(W) 0x%08x: 0x%08x\n", ioaddr, iodata);
				*(unsigned long*)(virt + (ioaddr - phys)) = iodata;
			}
		...
		if (opt_w) {
			...
			*(unsigned int*)(virt + (ioaddr - phys)) = iodata;
			...
			if ((ioaddr+length) >= (phys+size)) {
				printf("Fail, 0x%x io range 0x%08lx ~ 0x%lx \n", ioaddr, phys, (phys+size));
				goto _exit;
			}
...
static unsigned int iomem_map(unsigned long phys, unsigned int len)
{
	...
	virt = (unsigned long)mmap((void*)0, len,
				PROT_READ|PROT_WRITE, MAP_SHARED, fd, (off_t)phys);
	if (-1 == (int)virt) {
		printf("Fail: map phys=0x%08lx, len=%d, %s \n", phys, len, strerror(errno));
		goto _err;
	}

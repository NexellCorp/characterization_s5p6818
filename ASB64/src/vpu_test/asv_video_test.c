...
static uint32_t ReadOneFrame ( STREAM *pstStrm, VID_TYPE_E eCodec, int32_t iFrmNum )
{
	...
	if ( iFrmNum != 0 )
	{
		...
		else
		{
			...
			{
				int32_t uTmp  = (int32_t)(pstStrm->pbyStreamEnd - pbyStrm);
		...
		else
		{
			...
			do
			{
				...
				if ( uPreFourByte == 0x0000010D )
				{
					...
					pstStrm->uSize -= (uint32_t)(pstStrm->pbyStream - pstStrm->pbyStreamOrg);
					break;
				}
				uPreFourByte = (uPreFourByte << 8) + iOneByteEOF;
			} while ( 1 );
...
ASV_RESULT ASV_Video_Run(void)
{
	...
	do {
		stStrm.fpStream = fopen((const char*)stVidTestList[iTestCnt].chInputName, "rb");
		...
		// Initilize
		{
			...
			memset( &stSeqIn, 0, sizeof(stSeqIn) );

			stSeqIn.addNumBuffers = 4;
			stSeqIn.enablePostFilter = 0;
			stSeqIn.seqInfo = stStrm.pbyStream;
			stSeqIn.seqSize = uSize;
			stSeqIn.enableUserData = 0;
			stSeqIn.disableOutReorder = 0;

			if ( VID_ERR_NONE != NX_VidDecParseVideoCfg(hDec, &stSeqIn, &stSeqOut) )
			{
				printf("NX_VidDecParseVideoCfg() failed!!!\n");
				break;
			}

			stSeqIn.width = stSeqOut.width;
			stSeqIn.height = stSeqOut.height;

			if ( NX_VidDecInit(hDec, &stSeqIn) != VID_ERR_NONE )
			{
			...
			{
				NX_VID_ENC_INIT_PARAM stEncPara;
				memset(&stEncPara, 0, sizeof(stEncPara));

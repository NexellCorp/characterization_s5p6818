...
#include <linux/sched.h>		//	signal_pending / schedule_timeout
		...
		#include <mach/soc.h>
	#endif
#define	VPU_IRQ_ID		IRQ_PHY_CODA960_HOST
#define	JPU_IRQ_ID		IRQ_PHY_CODA960_JPG
#endif	//	__LINUX__
#else	//	64bit
	...
	#define	VPU_IRQ_ID		IRQ_CODA960_HOST
	#define	JPU_IRQ_ID		IRQ_CODA960_JPG
#endif
...
int JPU_InitInterrupt(void)
{
	...
	//	Initialize Interrupt
	if(request_irq( JPU_IRQ_ID, JPU_InterruptHandler, IRQF_DISABLED, "JPU_CODEC_IRQ", NULL) != 0)
	{
		NX_ErrMsg( ("%s:%d Cannot get IRQ %d\n", __func__, __LINE__, JPU_IRQ_ID ) );
		return -1;
	}
...
NX_VPU_RET NX_VpuJpegRunFrame( NX_VPU_INST_HANDLE handle, VPU_ENC_RUN_FRAME_ARG *runArg )
{
	...
	runArg->outStreamAddr = pInfo->strmBufVirAddr;

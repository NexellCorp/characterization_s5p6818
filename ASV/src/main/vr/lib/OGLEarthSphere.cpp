...
static GLboolean CheckResult(unsigned int frame_cout, FILE* fp);
...
void* base_runtime_grahpic_test_thread(void* arg)
{
	...
	sprintf((char*)sName, "./%s_%d_%dx%d_%d.data", "golden", nFrames, view_width, view_height, 4);

	fp = fopen((char*)sName, "r");	
	if(!fp)
	{
	...
	if(!nFrames)
	{
	  ...
	  {	  
		...
		if(SaveFrame(count))
		{
			//fwrite(gResultImage[count/TEST_GOLDEN_FRAME_INTERVAL], bytes_size , 1, fp);
			if(!CheckResult(count, fp))
			{
...
static GLboolean SaveFrame(unsigned int frame_cout)
{
	if(!(frame_cout % TEST_GOLDEN_FRAME_INTERVAL))
	{
